...
createServer({
    ...
    seeds(server) { //runs when the server starts
    ...
    routes() {  
        this.namespace = "api" //all routes will have /api at the beginning
        this.logging = false //to turn off the logging

        this.urlPrefix = 'http://localhost:3000';


        this.get("/vans", (schema, request) => {
            return schema.vans.all() //all the vans in the database
        })
